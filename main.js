Hooks.once("setup", async function() {
    ...
    game.coDragRuler = Macros;

	game.settings.register("coDragRuler", "globalDistances", {
    ...
    game.settings.register("coDragRuler", "walkDistance", {
    ...
    game.settings.register("coDragRuler", "runDistance", {
...
Hooks.once("dragRuler.ready", async function(SpeedProvider) {
    class coSpeedProvider extends SpeedProvider {
        get colors() {
            return [
        ...
        getRanges(token) {
            const global = game.settings.get("coDragRuler","globalDistances");
            const settingWalkDistance = game.settings.get("coDragRuler","walkDistance");
            const settingRunDistance = game.settings.get("coDragRuler","runDistance");

            const walkDistance = global ? settingWalkDistance : token.actor.data.data.moduleData?.coDragRuler?.distances?.walk;
            const runDistance = global ? settingRunDistance : token.actor.data.data.moduleData?.coDragRuler?.distances?.run;
    ...
    dragRuler.registerModule("coDragRuler", coSpeedProvider)
})
    ...
    static getDistances(){
        if (TokenLayer.instance.controlled.length > 1) ui.notifications.warn("Veuillez ne sélectionner qu'un seul token");
            ...
            tokens.forEach((token)=>{
                let walkDistance = token.actor.data.data.moduleData?.coDragRuler?.distances?.walk;
                let runDistance = token.actor.data.data.moduleData?.coDragRuler?.distances?.run;
